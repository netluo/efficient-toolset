...
class OrcHooks:
    ...
    def get_server_stat(self):
        ...
        self.cls_ins.clear()
        self.ins_info.clear()
        for cls_name in self.cls_name:
            # # 设置主的weight为0
            server_status = subprocess.getoutput(
                '{} -c topology -i {}'.format(self.orchestrator_client, cls_name))
    ...
    def zip_cls_info(self):
        self.cls_info.clear()
        self.get_server_stat()
        self.get_masters()
    ...
    def set_master_weight(self, ins_name):
    ...
    def set_slaver_weight(self, ins_name):
    ...
    def info_from_orc(self):
        self.zip_cls_info()
        # print(self.cls_info)
        try:
            for ins_info in self.cls_info:
                self.logger.info(ins_info)  # 记录节点状态信息
                delay_status: str = ins_info['delay']
                _is_readonly = ins_info['mysql_mode']
                _ins_name = ins_info['ins_name']
                ...
                # 如果是主,做判断，weight为0不用修改，如果不是0改为0
                    ...
                    # 设置主的weight为0
                    orchooks.set_master_weight(ins_name=_ins_name)
                # 如果节点是从库，并且不是延迟从库。设置weight为10
                elif _ins_name not in self.cls_master and ins_info['ins_stat'] != 'lag':
                    orchooks.set_slaver_weight(ins_name=_ins_name)
                ...
                if delay_status.rstrip('s').isnumeric():  # 如果延迟是数字类型
                    ...
                    if self.ins_delay_time > self.maxdelay:  # 如果延迟时间大于阈值
                        ...
                        self.monitor_haproxy(_ins_name, 'down')
                    elif ins_info['ins_stat'] == 'lag':  # 如果是延迟从库，一样配置为不可读。
                        self.monitor_haproxy(_ins_name, 'down')
                    else:
                        self.monitor_haproxy(_ins_name, 'up')

                else:  # 如果不是数字类型，可能是'unknown'等
                    self.logger.error('The instance {} is {} !'.format(
                        _ins_name, ins_info['ins_stat']))
                    self.monitor_haproxy(_ins_name, 'down')
            ...
            time.sleep(3)
            orchooks.orchook_start()

    def monitor_haproxy(self, delay_slaver_name, to_do):
        if to_do == 'down':
            ...
            if high_weight_rownum != 0:
                ...
                return self.degrade_slave(delay_slaver_name)
            ...
            if delay_slaver_name not in self.cls_name and low_weight_rownum == 1:
                return self.upgrade_slave(delay_slaver_name)

    def degrade_slave(self, slaver_name):
        try:
    ...
    def upgrade_slave(self, slaver_name):
        try:
    ...
    def orchook_start(self):
        self.hooksthread = threading.Timer(2, orchooks.info_from_orc)
