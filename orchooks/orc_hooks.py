...
class OrcHooks:
    def __init__(self):
        self.upgrade_slave_res = None
        self.hareload = None
        self.degrade_slave_res = None
        self.ins_delay_time = None
        self.delay_slaver_name = None
        self.logger = None
            ...
            for insinfo in self.cls_info:
                ...
                if delay_status.rstrip('s').isnumeric():  # 如果延迟是数字类型
                    ...
                    if self.ins_delay_time > self.maxdelay:  # 如果延迟时间大于阈值
                        ...
                        self.test_haproxy(self.delay_slaver_name, 'down')
                    else:
                        self.test_haproxy(self.delay_slaver_name, 'up')
    ...
    def test_haproxy(self, delay_slaver_name, to_do):
        if to_do == 'down':
    ...
    def upgrade_slave(self, slaver_name):
        try:
            self.upgrade_slave_res = subprocess.check_call(
